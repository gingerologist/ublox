...
port.on('data', data => {
  if (!timer) {
    timer = setTimeout(() => {
      log('discarding stale data')
      // log('discarding stale data ' + string.length + ' : ' + Buffer.from(string, 'binary').toString('hex'))
      if (string[0] === '$') {
        log(string)
      } else {
...
inputBar.on('submit', text => {
  if (text === 'help') {
    ...
    let body, tail, msg
    body = Buffer.from([0x06, 0x01, 0x08, 0x00, 0x01, 0x07, 0, 60, 0, 0, 0, 0])
    tail = ubxChecksum(body)
    msg = Buffer.concat([head, body, tail])
    body = Buffer.from([0x06, 0x01, 0x08, 0x00, 0x01, 0x04, 0, 60, 0, 0, 0, 0])
    tail = ubxChecksum(body)
